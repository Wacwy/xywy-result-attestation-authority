...
class TufRosterTests(unittest.TestCase):
    def setUp(self):
        ...
        self.private={k:ed25519.Ed25519PrivateKey.generate() for k in ('custodian-a','custodian-b')}
        ...
        self.roster={'schema_version':1,'initiative_id':'PGK-FAILCLOSED-001','candidate_generation':'v35','threshold':2,'authorities':[{'key_id':'authority-a','spki_sha256':'1'*64,'custodian_id':'custodian-a'},{'key_id':'authority-b','spki_sha256':'2'*64,'custodian_id':'custodian-b'}]}
        ...
        self.root_signed={'_type':'root','expires':'2030-01-01T00:00:00Z','keys':self.keys,'roles':{'root':{'keyids':['custodian-a','custodian-b'],'threshold':2},'targets':{'keyids':['custodian-a','custodian-b'],'threshold':2}},'spec_version':'1.0.31','version':1}
        self.emit()
    ...
    def emit(self,root_keys=('custodian-a','custodian-b'),target_keys=('custodian-a','custodian-b')):
    ...
    def test_two_of_two_accepts(self): self.assertTrue(self.verify()['verified'])
    ...
    def test_one_of_two_root_rejected(self):
        self.emit(root_keys=('custodian-a',))
        with self.assertRaisesRegex(ValueError,'threshold'): self.verify()
    def test_one_of_two_targets_rejected(self):
        self.emit(target_keys=('custodian-a',))
        with self.assertRaisesRegex(ValueError,'threshold'): self.verify()
    ...
    def test_same_custodian_label_rejected(self):
        bad=copy.deepcopy(self.roster); bad['authorities'][1]['custodian_id']='custodian-a'; raw=tr.canonical(bad); self.write('roster.json',bad)
        self.targets_signed['targets']['authority-roster.json']={'hashes':{'sha256':hashlib.sha256(raw).hexdigest()},'length':len(raw)}; self.emit()
        with self.assertRaisesRegex(ValueError,'distinct custodian'): self.verify()
